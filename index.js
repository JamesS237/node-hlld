...
hlld.prototype.serialize = function(cmd, args, cb) {
  if(!Array.isArray(args)) return cb(new Error('args should be an array'));
  if(!(typeof cmd == 'string')) return cb(new Error('command should be a string'));
...
var response_serialzier = function(cmd, data, cb) {
  ...
  if(data.toString('utf8').indexOf('Error') > -1 || data.toString('utf8').indexOf('Delete in progress') > -1) { //error!
    ...
    b.forEach(function(i) {
      var s = i.split(" "); //string version
      out[s[0]] = [s[1], s[2], s[3], s[4]].map(Number);
    });
    return cb(null, out);
  } else if(command('info')) { //info command
    var a = data.toString('utf8').split('\n');
...
Array.prototype.nthLast = function(n){return this[this.length-(n+1)]};
